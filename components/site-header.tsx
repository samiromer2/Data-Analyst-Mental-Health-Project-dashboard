  ...
  return (
    <header className="fixed top-0 left-0 z-50 flex h-16 w-full items-center justify-between border-b border-outline bg-surface-lowest px-4 md:px-10">
      <div className="flex items-center gap-8">
        <Link href="/" className="inline-flex items-center text-xl font-semibold text-primary">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`px-3 py-2 text-sm font-medium ${isActive(pathname, link.href)
        ...
        <nav className="absolute top-16 left-0 flex w-full flex-col border-b border-outline bg-surface-lowest px-4 py-3 md:hidden">
          {links.map((link) => (
            <Link
              ...
              className={`px-2 py-2 text-sm ${isActive(pathname, link.href) ? "text-primary" : "text-on-variant"
                }`}
            >
              {link.label}
            </Link>
