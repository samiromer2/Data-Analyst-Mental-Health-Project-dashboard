...
interface CanadaFlagProps extends React.SVGProps<SVGSVGElement> {
  className?: string;
}
...
export function CanadaFlag({ className = "h-4.5 w-auto", ...props }: CanadaFlagProps) {
  return (
    <svg
      ...
      role="img"
      className={`inline-block shrink-0 rounded-[2px] border border-black/15 shadow-xs transition-transform ${className}`}
      {...props}
    >
      <path fill="#ffffff" d="M150.1 0h339.7v480H150.1z" />
      <path
        fill="#d52b1e"
